...
- **2026-09-23** — Docs catch-up. Three live changes that were only in git are now recorded here: **21 Aug `aea3ad7`** reskin to the shared Seed design (token-level, no logic change); **25 Aug `64e854a`** logged-out UX — when the Microsoft session genuinely expires, a central "You have been logged out" popup appears and stale cached bay data is hidden behind the sign-in screen, instead of the old silent drop to offline mode (this closes the LOW finding about `acquireTokenPopup` degrading quietly on shared tablets); **26 Aug `faa4a38`** tokens aligned to the canonical Seed set (muted text `#5E6E64`, radius 10/14). Also on 23 Sep: this machine's working copy of the page had regressed to the `aea3ad7` version (live site unaffected) and was restored from `origin/main`. Review finding (open): the same ~46 KB logo JPEG is embedded twice as base64 (~124 KB of the 199 KB page) — could be one shared image.
