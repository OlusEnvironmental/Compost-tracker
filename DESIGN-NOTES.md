...
1. Strengthen `restoreJSON`: validate bay shape, show a diff/preview before committing, and auto-download a timestamped backup of the *current* data before overwriting. Highest-value remaining item — still a one-click way to replace live data with no backup.
2. Add `min`/basic sanity checks to the date inputs.
3. Prevent bay-name collisions at `confirmAddBay`.
4. Revisit the `confirmDelete` mid-lifecycle case — genuinely useful protection here likely means closing the cross-device staleness window further (more frequent/targeted re-sync, or a re-check immediately before the delete itself goes through) rather than a guard inside `confirmDelete` alone, which can't see past this device's own stale view of the world. Low urgency: the UI already blocks the straightforward single-device path to this.

## Changelog

- **2026-08-20** — Completed the Phase 1 as-is audit and wrote this document (previously no documentation existed for this system).
- **2026-08-20** — Fixed the three highest-value findings, one at a time with a check-in on each given this system's live-data status: the `turning-dest` status-badge display bug (`bb9ad5f`), a pre-save conflict check against concurrent SharePoint edits (`d5ba0c6`), and the 404-on-load silent data wipe — the audit's highest-severity finding (`5922512`). `restoreJSON` hardening (the last of the four originally agreed fixes) is still to come. The mid-lifecycle delete concern was re-assessed during implementation and found to be already blocked at the UI level for the single-device case described in the original audit; left open pending a proper fix for the underlying cross-device staleness window instead (see "Left open, deliberately").
